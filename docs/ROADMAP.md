...
| **현재 상태** | Phase 4 완료 — PDF 생성(invoice-pdf.tsx + Route Handler), 전역 에러 페이지, Noto Sans KR 폰트 임베딩, Vercel 배포 완료 |
...
| Phase 4 | PDF 생성 + 에러 처리 + Vercel 배포 | 1주 | 2026-04-24 | ✅ 완료 |
...
#### PDF 생성
- [x] `@react-pdf/renderer` 패키지 설치 (난이도: 낮음)
- [x] 한국어 폰트 임베딩 설정 (난이도: 중간)
  - Noto Sans KR `.ttf` 파일 다운로드 → `public/fonts/`
  - `Font.register()` 설정
- [x] PDF 문서 컴포넌트 `src/components/invoice/invoice-pdf.tsx` (난이도: 높음)
  - `Document`, `Page`, `View`, `Text`, `StyleSheet` 사용
  - 견적서 레이아웃 구현 (발행자 정보, 항목 테이블, 합계)
- [x] Route Handler `src/app/api/invoice/[slug]/pdf/route.tsx` (난이도: 중간)
...
- [x] 견적서 확인 페이지 PDF 다운로드 버튼 연결 (난이도: 낮음)

#### 에러 처리 및 UX 보완
- [x] 전역 에러 페이지 `src/app/error.tsx` (난이도: 낮음)
- [x] Notion API 호출 실패 시 에러 UI (난이도: 낮음)
- [x] 로딩 상태 전반적 검토 — Suspense 경계 점검 (난이도: 중간)
- [x] 메타데이터 설정 (`src/app/layout.tsx` — title, description) (난이도: 낮음)
- [x] 모바일(320px) ~ 데스크톱(1280px) 전 페이지 반응형 확인 (난이도: 중간)
- [x] 견적 항목 테이블 모바일 대응 (가로 스크롤 또는 카드형) (난이도: 중간)

#### Vercel 배포
- [x] GitHub 저장소 연결 및 Vercel 프로젝트 생성 (난이도: 낮음)
- [x] Vercel 환경변수 설정 (난이도: 낮음)
  - `NOTION_TOKEN`, `NOTION_DATABASE_ID`
- [x] `npm run build` 에러 없음 확인 (난이도: 낮음)
- [ ] 프로덕션 URL로 E2E 흐름 최종 검증 (난이도: 중간)
  - 대시보드 → URL 복사 → 클라이언트 접근 → 견적서 열람 → PDF 다운로드

### 완료 기준
- [x] PDF 다운로드 버튼 클릭 → 파일 다운로드
- [x] PDF 내 한국어 텍스트 정상 렌더링
...
| 2026-04-01 | v2.4.0 | Phase 4 구현 완료 — @react-pdf/renderer 기반 PDF 생성(invoice-pdf.tsx), Route Handler(/api/invoice/[slug]/pdf), 전역 에러 페이지(error.tsx), Noto Sans KR TTF 폰트 임베딩, GitHub 푸시 완료 |
