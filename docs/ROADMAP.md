...
|-------|------|------|------------|------|
| Phase 1 | 프로젝트 골격 (환경변수 + Notion 클라이언트 + 공유 타입) | 3일 | 2026-03-30 | 🔲 미시작 |
| Phase 2 | 핵심 기능 — 견적서 확인 페이지 (공개 URL + 항목 테이블) | 1주 | 2026-04-06 | 🔲 미시작 |
| Phase 3 | 대시보드 페이지 (견적서 목록 + URL 복사) | 1주 | 2026-04-13 | 🔲 미시작 |
...
- [ ] **PDF 다운로드** 버튼 플레이스홀더 (Phase 4에서 연결) (난이도: 낮음)
...
#### 대시보드 (`/dashboard`)
- [ ] 페이지 컴포넌트 `src/app/dashboard/page.tsx` (난이도: 낮음)
...
- [ ] 빈 상태 UI — Notion 환경변수 미설정 시 안내 메시지 (난이도: 낮음)
- [ ] 로딩 스켈레톤 `src/app/dashboard/loading.tsx` (난이도: 낮음)

#### 홈 페이지 업데이트
- [ ] `src/app/page.tsx` — 서비스 소개 + 대시보드 이동 버튼 (난이도: 낮음)
